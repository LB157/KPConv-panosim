...
class SemanticKittiDataset(PointCloudDataset):
	...
	#self.path = '../../Data/SemanticKitti'
        ...
        if self.set == 'training':
            #self.sequences = ['{:02d}'.format(i) for i in range(23,32) if i != 2]
            ''' 第三轮
            self.sequences = ['{:02d}'.format(i) for i in range(23,32)]  
            seA = ['{:02d}'.format(i) for i in range(1,6) if i != 2]
            self.sequences.extend(seA)
            '''
            
            '''
            # 第四轮
            self.sequences = ['{:02d}'.format(i) for i in range(4,6)]  
            seA = ['{:02d}'.format(i) for i in range(23,36) ]
            self.sequences.extend(seA)
            '''
            
            '''
            #第五轮
            self.sequences = ['{:02d}'.format(i) for i in range(4,7) if i != 5]  
            seA = ['{:02d}'.format(i) for i in range(23,38) ]   #9000
            self.sequences.extend(seA)
            '''
            '''
            #第六轮
            self.sequences = ['{:02d}'.format(i) for i in range(4,6)]  
            seA = ['{:02d}'.format(i) for i in range(23,39) ]  #9798
            self.sequences.extend(seA)   
            '''
            
             #第七轮
            self.sequences = ['{:02d}'.format(i) for i in range(4,6)]  
            seA = ['{:02d}'.format(i) for i in range(24,33) ]  #9798
            self.sequences.extend(seA)   
            #self.sequences = ['{:02d}'.format(i) for i in range(23,32) if i != 2] 
            # 1-6  !2  semantickitti:4934
            # 23-32 panosim:4846  
        elif self.set == 'validation':
            self.sequences = ['{:02d}'.format(i) for i in range(9,11)]
            #self.sequences = ['{:02d}'.format(i) for i in range(8,9)]
            #self.sequences = ['{:02d}'.format(i) for i in range(32,36)]
            #semantickitti :validation: 8  test:9.10
            #panosim:validation:32.33.34.35  test:semantickitti:9.10
    ...
    def __getitem__(self, batch_i):
        ...
        while True:
            ...
            while num_merged < self.config.n_frames and f_ind - f_inc >= 0:
                ...
                # Current frame pose
                #* s_ind是seq_index, f_ind是frame_index
            ...
            #* merged_point是在0帧坐标系下的采样后的点云
            #* merged_coords是在当前帧坐标系下的采样后的点云
            #* merged_labels点云标签
    ...
    def load_calib_poses(self):
        ...
        if self.set in ['training', 'validation']:
            ...
            self.class_frames = []
            #* 长度为20，如果类别在ignore里面，就加0进列表里
            #* 如果不是，就加包含这个类别的样本索引到列表里面
